...
``token_usage_utils.py`` modules. They all:
...
class TokenUsage:
    ...
    Attributes:
        ...
        input_audio_tokens: Audio input tokens (realtime/voice only).
        input_text_tokens: Text input tokens (realtime/voice only).
        input_cached_tokens: Cached input tokens (realtime/voice only).
        output_audio_tokens: Audio output tokens (realtime/voice only).
        output_text_tokens: Text output tokens (realtime/voice only).
    """
    ...
    def has_any(self) -> bool:
        """True if any token count is non-zero."""
...
def extract_usage(result: Any) -> Optional[TokenUsage]:
    """Extract usage from an agent_framework run result or ChatCompletion.
...
class TokenUsageEmitter:
    """Emit standardized token-usage custom events to Application Insights.

    Responsibilities:
        1. Emit LLM_Agent_Token_Usage, LLM_Model_Token_Usage, and
           LLM_Token_Usage_Summary events with consistent property schemas.
        2. Optionally sample high-cardinality events while always emitting
           the summary event for accurate per-request totals.
        3. Support per-model pricing for estimated cost calculation.
        4. Hash user_id values for PII/GDPR compliance when configured.

    Attributes:
        perf_slow_emit_threshold_ms: Soft threshold (ms) above which a
            WARNING is logged for an individual emit call.
    """
    ...
    def enabled(self) -> bool:
        """True when App Insights connection string and event sink are available."""
        return bool(self._cs) and self._sink is not None

    @property
    def sample_rate(self) -> float:
        """Current sampling rate for high-cardinality events."""
    ...
    def _summary_cost_props(
        ...
        """Best-effort cost for the summary event."""
        if primary_model:
    ...
    def emit(self, event_name: str, **dimensions: Any) -> None:
        """Low-level: emit an event with arbitrary properties. Never raises."""
        start_ns = time.perf_counter_ns()
        try:
            props = dict(self._static)
    ...
    def perf_stats(self) -> dict[str, float]:
        """Return cumulative telemetry-overhead stats.

        Returns:
            Dict with keys: emit_count, total_ms, avg_ms, max_ms.
        """
    ...
    def emit_agent(
        ...
        **dimensions: Any,
        ...
        """Emit a per-agent token usage event."""
    ...
    def emit_model(
        ...
        **dimensions: Any,
        ...
        """Emit a per-model token usage event."""
        if not usage.has_any or not self._should_sample():
            return
        self.emit(
            EVENT_MODEL,
    ...
    def emit_user(
        ...
        **dimensions: Any,
        ...
        """Emit a per-user token usage event."""
    ...
    def emit_team(
        ...
        **dimensions: Any,
        ...
        """Emit a per-team token usage event."""
    ...
    def emit_summary(
        ...
        **dimensions: Any,
        ...
        """Emit the summary event (always fires, ignores sample_rate)."""
    ...
    def emit_all(
        ...
        **dimensions: Any,
        ...
        """Emit summary, agent, and one model event per distinct model deployment.

        Args:
            agent_name: Name of the primary agent/step.
            model_deployment_name: Model deployment used by the primary agent.
            usage: Accumulated token usage for this invocation.
            additional_agents: Maps sub-agent name -> model deployment name.
            emit_user_event: Opt in to per-user events.
            emit_team_event: Opt in to per-team events.
            **dimensions: Extra properties forwarded to all events.
        """
        if not usage.has_any:
            return
...
class TokenUsageScope(AbstractContextManager):
    ...
    Example::
        ...
        with TokenUsageScope(emitter,
                             ...
                             process_id=pid) as scope:
            result = await agent.run(prompt)
            scope.add(result)

    Attributes:
        emitter: The TokenUsageEmitter instance to use for emission.
        agent_name: Name of the agent/step being tracked.
        model_deployment_name: Model deployment name for attribution.
        dimensions: Extra properties forwarded to all events.
        additional_agents: Maps sub-agent name -> model deployment name.
        emit_user_event: Whether to emit per-user events.
        emit_team_event: Whether to emit per-team events.
        usage: Accumulated TokenUsage so far.
    """
    ...
    def add(self, source: Any) -> Optional[TokenUsage]:
        ...
        Args:
            source: Agent run result, ChatMessage, or ChatCompletion object.

        Returns:
            The extracted TokenUsage, or None if extraction failed.
        """
        start_ns = time.perf_counter_ns()
        try:
    ...
    def add_usage(self, usage: TokenUsage) -> None:
        """Add a pre-constructed TokenUsage to the running total."""
        self.usage = self.usage + usage

    def add_chunks(self, chunks: Iterable[Any]) -> None:
        """Extract and accumulate usage from a stream of chunks."""
        for c in chunks:
            self.add(c)
