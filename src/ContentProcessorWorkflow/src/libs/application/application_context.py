...
This module implements a lightweight but full-featured DI framework built
around ``AppContext``.  It supports five service lifetimes — singleton,
transient, scoped, async-singleton, and async-scoped — and integrates
Azure ``DefaultAzureCredential`` for seamless cloud authentication.

Key classes:
    ServiceLifetime
        Enum-like constants (``SINGLETON``, ``TRANSIENT``, ``SCOPED``,
        ``ASYNC_SINGLETON``, ``ASYNC_SCOPED``) that describe how long a
        service instance lives.

    ServiceDescriptor
        Internal metadata record that pairs a service type with its
        implementation, lifetime, and cached instance reference.

    ServiceScope
        Context-manager wrapper returned by ``AppContext.create_scope()``
        to isolate scoped-service instances to a single request / task.

    AppContext
        The container itself — register services with ``add_*()``,
        resolve them with ``get_service()`` / ``get_service_async()``,
        and tear everything down via ``shutdown_async()``.

Typical usage::

    ctx = AppContext()
    ctx.set_configuration(Configuration())
    ctx.set_credential(DefaultAzureCredential())

    ctx.add_singleton(ILogger, ConsoleLogger)
    ctx.add_transient(IHandler, RequestHandler)
    ctx.add_async_singleton(ICache, RedisCache)

    logger = ctx.get_service(ILogger)
    cache  = await ctx.get_service_async(ICache)

    async with ctx.create_scope() as scope:
        handler = scope.get_service(IHandler)
...
class ServiceDescriptor:
    ...
    def __init__(
        ...
        """
        Initialise a new service descriptor.

        Args:
            service_type:    The service type / interface that consumers will
                             request from the container.
            implementation:  A class, factory callable, or pre-built instance
                             that satisfies *service_type*.
            lifetime:        One of the ``ServiceLifetime`` constants
                             (``SINGLETON``, ``TRANSIENT``, ``SCOPED``,
                             ``ASYNC_SINGLETON``, ``ASYNC_SCOPED``).
            is_async:        ``True`` when the service requires async
                             resolution (``get_service_async``).
            cleanup_method:  Name of the method called on disposal for async
                             services.  Defaults to ``"close"``.
        """
...
class ServiceScope:
    ...
    def get_service(self, service_type: Type[T]) -> T:
        """Resolve a synchronous service within this scope.

        Temporarily sets the parent container's scope context to this
        scope's ID, delegates to ``AppContext.get_service``, and restores
        the previous scope ID — even if resolution raises.

        Args:
            service_type: The type / interface to resolve.

        Returns:
            The service instance (cached for scoped lifetimes).

        Raises:
            KeyError: If *service_type* is not registered.
            ValueError: If the service is scoped but no scope is active.
        """
    ...
    async def get_service_async(self, service_type: Type[T]) -> T:
        """Resolve an async service within this scope.

        Behaves identically to ``get_service`` but awaits the parent
        container's ``get_service_async``, which handles async factory
        invocation and ``__aenter__`` initialisation.

        Args:
            service_type: The async service type / interface to resolve.

        Returns:
            The fully initialised async service instance.

        Raises:
            KeyError: If *service_type* is not registered.
            ValueError: If the service is not async or no scope is active.
        """
        old_scope = self._app_context._current_scope_id
        self._app_context._current_scope_id = self._scope_id
        try:
...
class AppContext:
    ...
    async def _cleanup_scope(self, scope_id: str):
        """
        Dispose every service instance cached under *scope_id*.

        Cleanup strategy (per service, in registration order):
            1. If the instance implements ``__aexit__``, call it directly
               (async context-manager protocol).
            2. Otherwise, if the instance exposes the descriptor's
               ``cleanup_method`` (default ``"close"``), call that —
               awaiting it when it is a coroutine.
            3. Synchronous services with no cleanup hook are simply
               discarded.

        After all instances have been torn down the scope entry is removed
        from ``_scoped_instances`` so no stale references survive.

        Args:
            scope_id: UUID string that identifies the scope to destroy.
        """
    ...
    async def _create_async_instance(self, descriptor: ServiceDescriptor) -> Any:
        """
        Instantiate an async service from its descriptor.

        Resolution order:
            1. **Pre-built instance** — if *implementation* is neither
               callable nor a type, return it as-is.
            2. **Factory callable / lambda** — invoke the callable; if it
               returns a coroutine, ``await`` it.  Then, if the resulting
               object implements ``__aenter__``, call that for async
               initialisation (e.g. opening a connection pool).
            3. **Class type** — call the no-arg constructor, then
               ``__aenter__`` if present.
            4. Anything else raises ``ValueError``.

        Args:
            descriptor: The ``ServiceDescriptor`` whose *implementation*
        ...
        Raises:
            ValueError: If the implementation type is not supported.
        """
    ...
    async def shutdown_async(self):
        """
        Gracefully shut down the container and release all async resources.

        Shutdown sequence:
            1. Cancel every outstanding task in ``_async_cleanup_tasks`` and
               ``await`` them (exceptions are suppressed).
            2. Iterate over all cached **async singleton** instances and invoke
               their configured ``cleanup_method`` (e.g. ``close``).
            3. Clear ``_instances``, ``_scoped_instances``, and the cleanup-task
               list so the container can be garbage-collected cleanly.

        This method is idempotent — calling it a second time is harmless
    ...
    def get_registered_services(self) -> Dict[Type, str]:
        ...
        Provides introspection into the dependency-injection container,
        returning every registered service type paired with its lifetime
        string.  Useful for debugging, health checks, and admin tooling.

        Returns:
            Dict[Type, str]: Service type to lifetime string mapping.
                Lifetime values: ``"singleton"``, ``"transient"``,
                ``"scoped"``, ``"async_singleton"``, ``"async_scoped"``.

        Example::

            for svc, lifetime in app_context.get_registered_services().items():
                print(f"{svc.__name__}: {lifetime}")
        """
        return {
