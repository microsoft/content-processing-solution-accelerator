...
def _get_message_role(message: Any) -> str | None:
    ...
    if isinstance(role, str):
        return role
    # agent_framework Role objects expose the string via .value
    val = getattr(role, "value", None)
    if isinstance(val, str):
        return val
    # Last resort: str() on Role returns the role name
    if role is not None:
        return str(role)
    return None
...
class ContextTrimConfig:
    ...
    enabled: bool = True
    # GPT-5.1 supports a 400K-token context window (~1.2M chars at ~3
    # chars/token for structured data).  Reserve ~128K tokens for output,
    # leaving ~272K input tokens ≈ ~800K chars.  The defaults guard against
    # accidental multi-hundred-KB blobs while giving enough room for the
    # large extracted-document payloads used in content-processing.
    max_total_chars: int = 800_000
    max_message_chars: int = 400_000
    keep_last_messages: int = 40
    keep_head_chars: int = 20_000
    keep_tail_chars: int = 6_000
    ...
    def from_env(
        ...
        return ContextTrimConfig(
            ...
            max_total_chars=max(0, _int(max_total_chars_env, 800_000)),
            max_message_chars=max(0, _int(max_message_chars_env, 400_000)),
            keep_last_messages=max(1, _int(keep_last_messages_env, 40)),
            keep_head_chars=max(0, _int(keep_head_chars_env, 20_000)),
            keep_tail_chars=max(0, _int(keep_tail_chars_env, 6_000)),
...
def _trim_messages(
    ...
    if cfg.keep_system_messages:
        for m in messages:
            if _get_message_role(m) == "system":
                ...
                break
        # Remove already-collected leading system messages from tail so they
        # are not duplicated when combined = system_messages + cleaned.
        tail = tail[len(system_messages) :]
    ...
    # Safety floor: never drop below 1 message.  If a single message exceeds
    # the budget, truncate it rather than returning an empty list.
    min_keep = max(1, len(system_messages))

    while len(combined) > min_keep and _total_chars(combined) > cfg.max_total_chars:
    ...
    # If the remaining messages still exceed the budget, truncate the
    # largest one (or the last one) to fit.
    if combined and _total_chars(combined) > cfg.max_total_chars:
        # Find the message contributing the most chars.
        largest_idx = max(
            range(len(combined)),
            key=lambda i: len(_estimate_message_text(combined[i])),
        )
        target_msg = combined[largest_idx]
        text = _estimate_message_text(target_msg)
        # Budget for this message = total budget minus all other messages.
        others_chars = sum(
            len(_estimate_message_text(combined[i]))
            for i in range(len(combined))
            if i != largest_idx
        )
        available = max(
            cfg.keep_head_chars + cfg.keep_tail_chars,
            cfg.max_total_chars - others_chars,
        )
        text = _truncate_text(
            text,
            max_chars=available,
            keep_head_chars=min(cfg.keep_head_chars, available),
            keep_tail_chars=min(cfg.keep_tail_chars, available),
        )
        combined[largest_idx] = _set_message_text(target_msg, text)
